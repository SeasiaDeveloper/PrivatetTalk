...
public class MyRoundedCornersTransformation extends BitmapTransformation {
    ...
    public MyRoundedCornersTransformation(Context context, int radius, int margin) {
        super(context);
        mContext = context;
        mRadius = radius;
        mDiameter = radius*2;
    ...
    protected Bitmap transform(BitmapPool pool, Bitmap toTransform, int outWidth, int outHeight) {
        ...
        int size = mContext.getResources().getDimensionPixelSize(R.dimen.chat_photo);

        Bitmap squared = ThumbnailUtils.extractThumbnail(toTransform, size, size);

        Bitmap bitmap = Glide.get(mContext).getBitmapPool().get(size, size, Bitmap.Config.ARGB_8888);
        if (bitmap == null) {
